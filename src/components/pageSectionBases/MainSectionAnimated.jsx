...
const MainSectionAnimated = forwardRef(
  ({ height, scrollBar, transformElements }, fullSectionRef) => {
    // hook for section y-scroll breakpoints (skip percentage end)
    const [sectionScrollPercentageStart, , sectionScrollStep] = useScrollPercentages(
      fullSectionRef
    );
    ...
    const calcAnimationScrollBreakpoints = (transformInfo) => {
      ...
      const transformStart =
        sectionScrollPercentageStart + sectionScrollStep * scrollPercentageStart;
...
MainSectionAnimated.displayName = 'MainSectionAnimated';
